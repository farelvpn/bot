...
async function handleTrialMenu(bot, query) {
    ...
    if (lastTrial) {
        ...
        if (now < cooldownEndTime) {
            ...
            await bot.telegram.answerCbQuery(query.id, `Anda sudah mengklaim trial. Coba lagi dalam ${timeLeft} jam.`, {
                show_alert: true,
            });
            return;
        }
    ...
    if (servers.length === 0) {
        return bot.telegram.answerCbQuery(query.id, 'Saat ini tidak ada server yang tersedia untuk trial.', { show_alert: true });
    }
...
async function processTrialClaim(bot, query) {
    ...
    if (lastTrial) {
        ...
        const cooldownEndTime = new Date(lastTrialTime.getTime() + TRIAL_COOLDOWN_HOURS * 60 * 60 * 1000);
        if (now < cooldownEndTime) {
            return bot.telegram.answerCbQuery(query.id, `Anda baru saja mengklaim trial.`, { show_alert: true });
        }
    }
